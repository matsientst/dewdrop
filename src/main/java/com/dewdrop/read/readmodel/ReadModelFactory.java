...
public class ReadModelFactory {
    ...
    public Optional<ReadModelConstructed> constructReadModel(Class<?> readModelTarget) {
        ...
        if (optInstance.isPresent()) {
            ...
            log.info("Created @ReadModel {} - ephemeral:{}", instance.getClass()
                .getSimpleName(), ReadModelUtils.isEphemeral(readModelTarget));
    ...
    <T extends Event> ReadModel<T> construct(Object target) {
        ...
        Stream[] streams = target.getClass()
            .getAnnotationsByType(Stream.class);
        if (streams.length == 0) {
            log.error("No @Stream annotation found on {} - This is used to know what stream to read from and is required", target.getClass()
                .getSimpleName());
            return null;
        }
        ...
        Arrays.stream(streams)
            .forEach(streamAnnotation -> {
                ...
                log.info("Creating Stream for stream:{} - subscribed:{} for ReadModel:{}", stream.getStreamDetails()
                    .getStreamName(), stream.getStreamDetails()
                    .isSubscribed(), target.getClass()
                    .getSimpleName());
            });
        return readModel;
    }
    ...
    public <T extends Event> com.dewdrop.read.readmodel.ReadModel<T> createReadModelForOnEvent(Method annotatedMethod) {
        ...
        if (onEventService.isEmpty()) {
            log.error("Unable to construct read model for @OnEvent method:{} in target class:{}", annotatedMethod.getName(), annotatedMethod.getDeclaringClass()
                .getSimpleName());
            return null;
        }
        ...
        if (params.length == 0) {
            log.error("Invalid @OnEvent {}.{}() - First parameter must have a parameter that extends com.dewdrop.structure.api.Event like handle(ExampleAccountCreated event)", annotatedMethod.getDeclaringClass()
                    .getSimpleName(),
                annotatedMethod.getName());
        }
        Class<?> eventType = params[0];
        if (!Event.class.isAssignableFrom(eventType)) {
            log.error("Invalid first parameter:{} for @OnEvent {}.{}({} event) - First parameter must extend com.dewdrop.structure.api.Event", eventType.getSimpleName(), annotatedMethod.getDeclaringClass()
                    .getSimpleName(), annotatedMethod.getName(),
                eventType.getSimpleName());
        }
        log.info("Created @OnEvent ReadModel {} for eventType:{}", instance.getClass()
            .getSimpleName(), eventType.getSimpleName());

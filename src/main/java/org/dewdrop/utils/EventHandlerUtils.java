    ...
    public static Map<Class<? extends Event>, Method> getEventToHandlerMethod(Class<?> readModelClass, Class<? extends Annotation> annotationClass) {
        return DewdropAnnotationUtils.getAnnotatedMethods(readModelClass, annotationClass).stream().filter(method -> {
            ...
            if (!noParameter) {
                ...
                log.error("The method annotated with @EventHandler {}.{}() has no parameter and is invalid and cannot handle any events. Please add the event you want to handle like {}(ExampleEvent event) where ExampleEvent extends org.dewdrop.structure.api.Event as the first parameter",
            ...
            if (!isAssignable) {
                String methodName = method.getName();
                log.error("The method annotated with @EventHandler {}.{}({} event) has a parameter that is not an event. Please add the event you want to handle like {}(ExampleEvent event) where ExampleEvent extends org.dewdrop.structure.api.Event as the first parameter",

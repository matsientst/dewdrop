...
class DefaultAnnotationReadModelMapperTest {
    ...
    void registerReadModels() {
        ...
        ReadModel mockedReadModel = mock(ReadModel.class);
        doReturn(mockedReadModel).when(readModelConstructed).getReadModel();
    ...
    @Disabled
    // Throwing stackoverflow for some reason
    ...
    @events.dewdrop.read.readmodel.annotation.ReadModel(ephemeral = true, destroyInMinutesUnused = events.dewdrop.read.readmodel.annotation.ReadModel.NEVER_DESTROY)
    private class NeverDestroyReadModel {
        public NeverDestroyReadModel() {}
    }

    @events.dewdrop.read.readmodel.annotation.ReadModel(ephemeral = true, destroyInMinutesUnused = 60)

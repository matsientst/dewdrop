...
class StreamTest {
    ...
    void setup() {
        ...
        streamDetails = spy(StreamDetails.builder().streamType(StreamType.AGGREGATE).direction(Direction.FORWARD).aggregateName(new AggregateRoot().getClass().getSimpleName()).streamNameGenerator(streamNameGenerator).id(UUID.randomUUID())
    ...
    void updateQueryState() {
        ...
        stream.updateQueryState(Optional.empty());
    ...
    void updateQueryState_subscribed() {
        doReturn(true).when(streamDetails).isSubscribed();
        stream.updateQueryState(Optional.empty());

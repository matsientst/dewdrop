...
class StreamTest {
    ...
    void setup() {
        ...
        streamDetails = spy(StreamDetails.builder().streamType(AGGREGATE).direction(Direction.FORWARD).aggregateRoot(new AggregateRoot()).streamNameGenerator(streamNameGenerator).id(UUID.randomUUID()).subscribed(true)
        ...
        stream = spy(new Stream(streamDetails, streamStore, eventSerializer));
    ...
    void getById() {
        ...
        doReturn(AGGREGATE).when(streamDetails).getStreamType();
    ...
    void getById_wrongStreamType() {
        StreamStoreGetByIDRequest request = new StreamStoreGetByIDRequest(aggregateRoot, UUID.randomUUID(), 1, new DewdropAddFundsToAccountCommand(UUID.randomUUID(), new BigDecimal(100)));
        doReturn(CATEGORY).when(streamDetails).getStreamType();

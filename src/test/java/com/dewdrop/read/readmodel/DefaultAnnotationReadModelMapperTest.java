...
import static com.dewdrop.read.readmodel.annotation.ReadModel.NEVER_DESTROY;
...
class DefaultAnnotationReadModelMapperTest {
    ...
    ReadModel readModel = new ReadModel(new DewdropAccountDetailsReadModel(), null);
    ...
    void createAndCacheEphemeralReadModel_keepIndefinitely() {
        ...
        ReadModelConstructed constructed = new ReadModelConstructed(new ReadModel<>(new NeverDestroyReadModel(), null));
        doReturn(Optional.of(constructed)).when(readModelFactory).constructReadModel(any(Class.class));
    ...
    void createAndCacheEphemeralReadModel_keepForAnHour() {
        ReadModelConstructed constructed = new ReadModelConstructed(new ReadModel<>(new KeepForAnHourReadModel(), null));
        doReturn(Optional.of(constructed)).when(readModelFactory).constructReadModel(any(Class.class));
    ...
    @com.dewdrop.read.readmodel.annotation.ReadModel(ephemeral = true, destroyInMinutesUnused = NEVER_DESTROY)
    private class NeverDestroyReadModel {
    }

    @com.dewdrop.read.readmodel.annotation.ReadModel(ephemeral = true, destroyInMinutesUnused = 60)

...
class StandaloneAggregateProcessorTest {
    ...
    void setup() {
        ...
        standaloneAggregateProcessor = spy(new StandaloneAggregateProcessor(streamFactory));
    ...
    void save() {
        doReturn(stream).when(streamFactory).constructStream(any(AggregateRoot.class), any(UUID.class));

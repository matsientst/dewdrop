...
class ReadModelFactoryTest {
    ...
    void construct() {
        org.dewdrop.read.readmodel.stream.Stream stream = mock(org.dewdrop.read.readmodel.stream.Stream.class);
        ...
        doReturn(stream).when(streamFactory).constructStreamFromStream(any(org.dewdrop.read.readmodel.annotation.Stream.class), any(ReadModel.class));
        doNothing().when(readModel).addStream(any(org.dewdrop.read.readmodel.stream.Stream.class));
        try (MockedStatic<ReadModelUtils> utilities = mockStatic(ReadModelUtils.class)) {
            ...
            try (MockedStatic<EventHandlerUtils> eventUtilities = mockStatic(EventHandlerUtils.class)) {
                ...
                verify(readModel, times(2)).addStream(any(org.dewdrop.read.readmodel.stream.Stream.class));
            }
        }
    }
